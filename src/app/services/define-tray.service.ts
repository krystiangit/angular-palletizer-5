...
export class DefineTrayService {
...
this.fetchData('/api/trays-of-pallet').then((data) => {
  this.test = data;
  this.boxesOfPallet = data;
      });
this.fetchData('/api/trays-of-pp').then((data) => {
        this.test1 = data;
      });
      resolve();
    });
  ...
  postBoxes() {
    console.log("post boxes of PP:")
    console.log(this.boxesOfPickingPlace)
    const httpOptions = {
    ...
    const urlBoxesOfPallet = '/api/trays-of-pallet';
    const urlBoxesOfPp = '/api/trays-of-pp';
    this.httpClient
  ...
  addBox() {
    ...
    if (this.boxSets.membership.search('Pallet') == 0) {
      temp.name = 'TrayOfPallet' + (this.boxesOfPallet.length + 1);
      temp.id = (this.boxesOfPallet.length + 1).toString();
      this.boxesOfPallet.push(temp);
      return this.boxesOfPallet;
    }
    if (this.boxSets.membership.search('Picking') == 0) {
      temp.name = 'TrayOfPp' + (this.boxesOfPickingPlace.length + 1);
